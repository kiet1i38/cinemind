import { ArrowLeft, ArrowUpRight, Clock, Star } from "@phosphor-icons/react";
...
export function DetailView({ item, related, language, onBack, onRate, onSelect }) {
  if (!item) {
    return <main className="detail-page not-found-page"><button type="button" className="back-button" onClick={onBack}><ArrowLeft size={18} aria-hidden="true" />{translate(language, "backToBrowse")}</button><div className="empty-state"><h1>{translate(language, "titleNotFound")}</h1><p>{translate(language, "titleNotFoundDescription")}</p></div></main>;
  }
  return (
      ...
      <div className="detail-inner">
        ...
        <section className="detail-hero" aria-labelledby="detail-heading">
          ...
          <div className="detail-copy">
            ...
            <div className="detail-meta"><span className="detail-rating"><Star size={15} weight="fill" aria-hidden="true" />{item.rating ?? translate(language, "notListed")}</span><span><Clock size={15} weight="bold" aria-hidden="true" />{getRuntimeLabel(item, language)}</span></div>
            <p className="detail-description">{item.description || translate(language, "noData")}</p>
            <div className="detail-actions"><button type="button" className="primary-button" onClick={() => onRate(item)}>{translate(language, "addToSignals")} <ArrowUpRight size={17} weight="bold" aria-hidden="true" /></button></div>
        ...
        <TitleRail id="related-detail" title={translate(language, "previewPicks")} description={translate(language, "previewPicksDescription")} items={related} language={language} onSelect={onSelect} />
      </div>
    </main>
  );
}
